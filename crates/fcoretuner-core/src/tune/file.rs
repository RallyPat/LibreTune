...
impl TuneFile {
    ...
    fn parse_msq(content: &str) -> io::Result<Self> {
        ...
        // Parse iniMetadata section (FCoreTuner 1.1+ format for version tracking)
        ...
        // Parse constantManifest section (FCoreTuner 1.1+ format for migration detection)
    ...
    fn save_msq<P: AsRef<Path>>(&self, path: P) -> io::Result<()> {
        ...
        // Add INI metadata for version tracking (FCoreTuner 1.1+)
        ...
        // Add constant manifest for migration detection (FCoreTuner 1.1+)
    ...
    pub fn save_pc_variables<P: AsRef<Path>>(&self, path: P, signature: &str) -> io::Result<()> {
        ...
        xml.push_str("<msq xmlns=\"http://www.msefi.com/:msq\">\n");
        xml.push_str(&format!(
            "<bibliography author=\"FCoreTuner\" writeDate=\"{}\"/>\n",
...
<msq version="1.0">
  <bibliography author="FCoreTuner Test" tuneComment="Test tune" writeDate="2026-01-07"/>
...
<!DOCTYPE msq PUBLIC "-//rusEfi//DTD msq//EN" "msq.dtd">
<msq version="1.0">
  <bibliography author="FCoreTuner Test" tuneComment="Test tune" writeDate="2026-01-11"/>
  <versionInfo firmwareInfo="rusEFI test"/>

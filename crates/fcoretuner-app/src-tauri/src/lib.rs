...
use fcoretuner_core::protocol::serial::list_ports;
use fcoretuner_core::protocol::{Connection, ConnectionConfig, ConnectionState};
use fcoretuner_core::table_ops;
use fcoretuner_core::tune::{
...
/// Get the FCoreTuner app data directory (cross-platform)
...
fn convert_layout_to_dashfile(layout: &DashboardLayout) -> DashFile {
    use fcoretuner_core::dash::{BackgroundStyle, GaugeCluster};
    use fcoretuner_core::dashboard::GaugeType;

    let mut dash = DashFile {
        bibliography: Bibliography {
            author: "FCoreTuner".to_string(),
            company: "FCoreTuner".to_string(),
        ...
        Ok(def) => {
            ...
            *cache_guard = Some(cache);
            ...
            if let Some(tune) = current_tune {
                ...
                use fcoretuner_core::tune::TuneValue;
                ...
                for (name, tune_value) in &tune.constants {
                    if let Some(constant) = def_clone.constants.get(name) {
                        ...
                        if constant.data_type == fcoretuner_core::ini::DataType::Bits {
...
async fn connect_to_ecu(
    ...
    if baud_rate == 0 {
        eprintln!(
            ...
            fcoretuner_core::protocol::DEFAULT_BAUD_RATE
        );
        config.baud_rate = fcoretuner_core::protocol::DEFAULT_BAUD_RATE;
    } else {
...
async fn get_table_data(
    ...
    fn read_const_from_source(
        ...
        endianness: fcoretuner_core::ini::Endianness,
        ...
        if let Some(ref mut conn_ptr) = conn {
            ...
            let params = fcoretuner_core::protocol::commands::ReadMemoryParams {
                can_id: 0,
                page: constant.page,
...
async fn update_table_data(
    ...
    if let Some(conn) = conn_guard.as_mut() {
        let params = fcoretuner_core::protocol::commands::WriteMemoryParams {
            can_id: 0,
            page: constant.page,
            offset: constant.offset,
            data: raw_data.clone(),
        ...
        if cache.write_bytes(constant.page, constant.offset, &raw_data) {
            ...
            if let Some(tune) = tune_guard.as_mut() {
                ...
                tune.constants.insert(
                    constant.name.clone(),
                    fcoretuner_core::tune::TuneValue::Array(y_values.clone()),
                );
    ...
    Ok(def.frontpage.as_ref().map(|fp| FrontPageInfo {
        ...
        indicators: fp
            ...
            .map(|ind| FrontPageIndicatorInfo {
                ...
                bg_off: fcoretuner_core::ini::FrontPageIndicator::color_to_css(&ind.bg_off),
                fg_off: fcoretuner_core::ini::FrontPageIndicator::color_to_css(&ind.fg_off),
                bg_on: fcoretuner_core::ini::FrontPageIndicator::color_to_css(&ind.bg_on),
                fg_on: fcoretuner_core::ini::FrontPageIndicator::color_to_css(&ind.fg_on),
            })
            .collect(),
    }))
}
    ...
    match item {
        ...
        | MenuItem::Help {
            ...
            if let Some(vis_cond) = visibility_condition {
                let mut parser = fcoretuner_core::ini::expression::Parser::new(vis_cond);
                if let Ok(expr) = parser.parse() {
                    if let Ok(val) =
                        fcoretuner_core::ini::expression::evaluate_simple(&expr, context)
                    {
                        if !val.as_bool() {
            ...
            if let Some(en_cond) = enabled_condition {
                let mut parser = fcoretuner_core::ini::expression::Parser::new(en_cond);
                if let Ok(expr) = parser.parse() {
                    if let Ok(val) =
                        fcoretuner_core::ini::expression::evaluate_simple(&expr, context)
                    {
...
async fn evaluate_expression(
    ...
    let mut parser = fcoretuner_core::ini::expression::Parser::new(&expression);
    let expr = parser.parse().map_err(|e| e)?;
    let val = fcoretuner_core::ini::expression::evaluate_simple(&expr, &context).map_err(|e| e)?;
    Ok(val.as_bool())
}
...
async fn get_indicator_panel(
    state: tauri::State<'_, AppState>,
    name: String,
) -> Result<fcoretuner_core::ini::IndicatorPanel, String> {
    let def_guard = state.definition.lock().await;
    let def = def_guard.as_ref().ok_or("Definition not loaded")?;
...
async fn get_port_editor(
    state: tauri::State<'_, AppState>,
    name: String,
) -> Result<fcoretuner_core::ini::PortEditorConfig, String> {
    let def_guard = state.definition.lock().await;
    let def = def_guard.as_ref().ok_or("Definition not loaded")?;
    def.port_editors
        .get(&name)
    ...
    let value_type = match constant.data_type {
        ...
        _ => {
            ...
            match &constant.shape {
                fcoretuner_core::ini::Shape::Scalar => "scalar".to_string(),
                _ => "array".to_string(),
            }
        }
    };
...
async fn update_constant(
    ...
    if constant.is_pc_variable {
        ...
        if let Some(tune) = tune_guard.as_mut() {
            tune.constants
                .insert(name, fcoretuner_core::tune::TuneValue::Scalar(value));
        }
        return Ok(());
    }
    ...
    if constant.data_type == fcoretuner_core::ini::DataType::Bits {
        ...
        if let Some(tune) = tune_guard.as_mut() {
            ...
            tune.constants
                .insert(name.clone(), fcoretuner_core::tune::TuneValue::Scalar(value));
        }
...
async fn get_all_constant_values(
    ...
    for (name, constant) in &def.constants {
        ...
        if !matches!(constant.shape, fcoretuner_core::ini::Shape::Scalar) {
            continue;
        }
...
async fn send_autotune_recommendations(
    ...
    // Write back to ECU
    let write_params = fcoretuner_core::protocol::commands::WriteMemoryParams {
        can_id: 0,
        page: constant.page,
        offset: constant.offset,
        data: raw_out,
...
/// Create a FCoreTuner default dashboard
#[tauri::command]
...
/// Create a basic dashboard layout - FCoreTuner default
...
async fn save_tune(
    ...
    // Populate INI metadata for version tracking (FCoreTuner 1.1+)
...
async fn load_tune(
    ...
    // Check for INI version migration if tune has a saved manifest (FCoreTuner 1.1+ tunes)
    // This helps users understand what changed between INI versions
    {
        use fcoretuner_core::tune::migration::compare_manifests;
...
fn read_table_values(
    cache: &TuneCache,
    def: &EcuDefinition,
    table_def: &fcoretuner_core::ini::TableDefinition,
    rows: usize,
    cols: usize,
) -> Result<Vec<f64>, String> {
...
async fn preview_tunerstudio_import(path: String) -> Result<TsImportPreview, String> {
    use fcoretuner_core::project::Properties;

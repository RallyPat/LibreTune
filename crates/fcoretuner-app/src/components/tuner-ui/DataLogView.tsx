...
export const DataLogView: React.FC<DataLogViewProps> = ({ realtimeData }) => {
  ...
  // Parse CSV file - supports both FCoreTuner and TunerStudio formats
  const parseLogCsv = useCallback((content: string, _fileName: string): { 
    ...
    // Detect format: TunerStudio uses "Time" column, FCoreTuner uses "timestamp_ms"
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (timeColIndex >= 0) {
        ...
        if (isTunerStudioFormat) {
          ...
          // FCoreTuner format: milliseconds

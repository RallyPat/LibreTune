...
function DialogField({ 
      ...
      <div className="settings-field">
        <label>
          ...
          {(showAllHelpIcons || constant.help) && (
            <span className="help-icon" title={constant.help || '点击查看信息'} onClick={handleFocus} tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && handleFocus()}>
              <HelpCircle size={16} />
            </span>
          )}
        </label>
  ...
  if (constant.value_type === 'bits') {
    ...
    if (validBitOptions.length === 2) {
      ...
      const uncheckedLabel = bitOptions[uncheckedIndex]?.trim() || '关';
      const checkedLabel = bitOptions[checkedIndex]?.trim() || '开';
...
const COMMAND_WARNINGS_DISABLED_KEY = 'fcoretuner_command_warnings_disabled';
        ...
        <div className="command-warning-overlay" onClick={() => setShowWarning(false)}>
          <div className="command-warning-dialog" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="command-warning-buttons">
              ...
              <button onClick={() => handleWarningConfirm(false)}>执行一次</button>
...
function RecursivePanel({
  ...
  if (panelType === 'loading') {
    return <div className="panel-loading">正在加载 {name}...</div>;
  }
  ...
  if (panelType === 'table' && tableInfo) {
    return (
      <div className="embedded-table-link" onClick={() => openTable(name)}>
        <Grid3X3 size={20} />
        <span>打开表格: {tableInfo.title || name}</span>
      </div>
    );
  }
  ...
  if (comp.type === 'LiveGraph') {
    return (
      <div className="embedded-graph-placeholder">
        <Activity size={20} />
        <span>实时图表: {comp.title || comp.name}</span>
      </div>
    );
  }

...
function AppContent() {
    ...
    try {
      ...
      if (!ports.includes(selectedPort)) {
        if (ports.length > 0) {
          ...
          showToast(`所选端口 '${old}' 不可用，改用 '${ports[0]}'。`, "warning");
        } else {
  ...
  function formatError(e: unknown): { message: string; details: string } {
    ...
    if (errorStr.includes("panicked") || errorStr.includes("overflow") || errorStr.includes("thread")) {
      return {
        message: "处理调校文件时发生内部错误。这可能表示 INI 定义与调校文件不兼容。",
        details: errorStr,
      };
    }
    ...
    if (errorStr.includes("parse") || errorStr.includes("Parse") || errorStr.includes("invalid")) {
      return {
        message: "无法解析调校文件。文件可能已损坏或使用了不支持的格式。",
        details: errorStr,
      };
    }
  ...
  async function importIniToRepository() {
    try {
      const selected = await open({
        multiple: false,
        filters: [{ name: "INI 定义文件", extensions: ["ini"] }],
      });
  ...
  const openTarget = useCallback(
    async (name: string, title?: string) => {
      ...
      try {
        ...
        showToast(`无法打开 "${title || name}" - 未找到对应的表格、曲线、对话框或端口编辑器`, "warning");
      }
    },
      ...
      switch (target) {
        case "std_realtime":
          ...
          setTabs(prev => {
            ...
            return [{ id: "dashboard", title: "仪表盘", icon: "dashboard", closable: false }, ...prev];
          });
          setTabContents(prev => {
    ...
    async (command: string, featureName: string, args?: Record<string, unknown>) => {
      try {
        ...
        if (errMsg.includes("not yet implemented")) {
          showToast(`${featureName} - 即将推出！`, "info");
        } else {
          showToast(`${featureName} 失败：${errMsg}`, "error");
        }
      }
    },
    [showToast]
  ...
  const menuItems: TunerMenuItem[] = useMemo(() => {
    ...
    const toolsMenu: TunerMenuItem = {
      ...
      label: "&工具",
      items: [
        { id: "autotune", label: "&自动调教", onClick: () => openTarget("autotune", "自动调校实时"), disabled: !currentProject },
        { id: "datalog", label: "&数据日志", onClick: () => openTarget("datalog", "数据记录"), disabled: !currentProject },
        { id: "sep1", label: "", separator: true },
        { id: "tooth-logger", label: "&齿形日志", onClick: () => openTarget("tooth-logger", "齿形记录器"), disabled: !currentProject },
        { id: "composite-logger", label: "&复合日志", onClick: () => openTarget("composite-logger", "复合记录器"), disabled: !currentProject },
        { id: "sep2", label: "", separator: true },
        ...
        { id: "import-csv", label: "从 CSV &导入调校", onClick: async () => {
          try {
            ...
            if (filePath && typeof filePath === "string") {
              ...
              showToast(`从 CSV 导入了 ${count} 个值`, "success");
            }
          } catch (err) {
    ...
    const helpMenu: TunerMenuItem = {
      ...
      items: [
        ...
        { id: "sep1", label: "", separator: true },
        { id: "about", label: "&关于FCoreTuner软件", onClick: () => setAboutDialogOpen(true) },
      ],
    };
    ...
    if (syncStatus && syncStatus.pages_failed > 0) {
      items.push({
        ...
        content: (
          ...
          >
            ⚠ 部分同步 ({syncStatus.pages_synced}/{syncStatus.total_pages})
          </span>
        ),
      });
    }
      ...
      case "portEditor":
        return (
          <div className="port-editor-placeholder" style={{ padding: 24 }}>
            <h2>{(content.data as PortEditorConfig)?.label || "端口编辑器"}</h2>
            <p>端口编辑器用于：{(content.data as PortEditorConfig)?.name || "未知"}</p>
            <p style={{ color: 'var(--text-muted)', marginTop: 8 }}>
              <em>端口编辑器 UI 尚未实现。此功能允许配置 ECU 引脚分配。</em>
            </p>
          </div>
        );
      case "settings":
    ...
    <div style={{ padding: 20 }}>
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          ...
          >
            {demoLoading ? '加载中...' : demoMode ? '禁用' : '启用'}
          </button>
        </div>
        <p style={{ 
          ...
          ⚠️ 这会生成<strong>模拟传感器数据</strong>用于界面测试。
          您<strong>未连接到真实的 ECU</strong>。
          模拟引擎在约 850 RPM 怠速，偶尔有油门波动。
        </p>
      </div>
            ...
            {projects.slice(0, 5).map((project) => (
              ...
              >
                ...
                <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4 }}>
                  最后修改：{new Date(project.modified).toLocaleDateString()}
                </div>
              </div>
            ))}
          </div>
  ...
  async function browseTune() {
    try {
      const path = await open({
        multiple: false,
        filters: [
          { name: "调校文件", extensions: ["xml", "msq"] },
          { name: "FCoreTuner 调校", extensions: ["xml"] },
        ...
        <div style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 16, marginBottom: 24 }}>
            ...
            >
              ...
              <div style={{ fontWeight: 600 }}>从头开始</div>
              <div style={{ fontSize: 12, color: "var(--text-muted)" }}>手动选择 INI</div>
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <input
              ...
              placeholder={`我的 ${selectedTemplate.ecu_type} 调校`}
              style={{
                width: "100%",
                padding: 12,
                borderRadius: 6,
          ...
          <div style={{ 
            ...
            <div style={{ fontWeight: 500, marginBottom: 12 }}>模板设置</div>
            <div style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: "8px 16px" }}>
              <span style={{ color: "var(--text-muted)" }}>ECU 签名：</span>
              <span>{selectedTemplate.ini_signature}</span>
              <span style={{ color: "var(--text-muted)" }}>波特率：</span>
              <span>{selectedTemplate.connection_baud_rate}</span>
              <span style={{ color: "var(--text-muted)" }}>仪表盘：</span>
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <label style={{ fontWeight: 500 }}>ECU 定义</label>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <label style={{ fontWeight: 500 }}>导入现有调校（可选）</label>

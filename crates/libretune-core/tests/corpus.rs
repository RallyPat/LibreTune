...
fn test_parse_all_corpus_inis() {
    ...
    if !corpus_path.exists() {
        println!("Corpus directory not found at {:?}, skipping test", corpus_path);
        return;
    }
    ...
    println!("Files passed: {} ({:.1}%)", files_passed, 
...
fn test_rusefi_ini_fields() {
    ...
    if let Some(path) = rusefi_file {
        let def = EcuDefinition::from_file(&path)
            .expect("Should parse rusEFI INI");
        
        // rusEFI files should have VeAnalyze section
        assert!(def.ve_analyze.is_some(), "rusEFI should have VeAnalyze section");
        ...
        assert!(!def.output_channels.is_empty(), "Should have output channels");
        ...
        println!("  MaintainConstantValue entries: {}", def.maintain_constant_values.len());
    }
}
...
fn test_speeduino_ini_fields() {
    ...
    if let Some(path) = speeduino_file {
        let def = EcuDefinition::from_file(&path)
            .expect("Should parse Speeduino INI");

...
fn test_parse_basic_dash() {
    ...
    assert!(result.is_ok(), "Failed to parse basic.dash: {:?}", result.err());
    ...
    let gauge_count = components.iter().filter(|c| matches!(c, DashComponent::Gauge(_))).count();
    let indicator_count = components.iter().filter(|c| matches!(c, DashComponent::Indicator(_))).count();
    
    // Verify component counts (should have 4 gauges and 2 indicators)
    assert_eq!(gauge_count, 4, "Expected 4 gauges, got {}", gauge_count);
    assert_eq!(indicator_count, 2, "Expected 2 indicators, got {}", indicator_count);
    
    // Check RPM gauge
    let rpm_gauge = components.iter()
    ...
    // Check coolant gauge
    let coolant_gauge = components.iter()
        .filter_map(|c| match c {
    ...
    assert!(matches!(coolant_gauge.gauge_painter, GaugePainter::VerticalBarGauge), 
        "Expected VerticalBarGauge, got {:?}", coolant_gauge.gauge_painter);
    assert_eq!(coolant_gauge.high_warning, Some(100.0));
    assert_eq!(coolant_gauge.high_critical, Some(110.0));
    
    // Check AFR gauge
    let afr_gauge = components.iter()
        .filter_map(|c| match c {
    ...
    assert!(matches!(afr_gauge.gauge_painter, GaugePainter::BasicReadout));
    assert_eq!(afr_gauge.value_digits, 2);
    
    // Check MAP gauge
    let map_gauge = components.iter()
        .filter_map(|c| match c {
    ...
    assert!(matches!(map_gauge.gauge_painter, GaugePainter::AsymmetricSweepGauge));
    
    // Check battery indicator
    let battery_ind = components.iter()
        .filter_map(|c| match c {
...
fn test_color_parsing() {
    ...
    let rpm_gauge = dash.gauge_cluster.components.iter()
        .filter_map(|c| match c {
            DashComponent::Gauge(g) if g.output_channel == "rpm" => Some(g.as_ref()),
            _ => None,
        })
    ...
    assert_ne!(rpm_gauge.needle_color.red, 0, "Needle color should be red, not black");
    
    // Font color is white (-1 = 0xFFFFFFFF), so it should have non-zero values
    assert_ne!(rpm_gauge.font_color.red, 0, "Font color should be white, not black");
}
...
fn test_gauge_painter_types() {
    ...
    let painters: Vec<_> = dash.gauge_cluster.components.iter()
        .filter_map(|c| match c {
    ...
    assert!(painters.iter().any(|p| matches!(p, GaugePainter::Tachometer)));
    assert!(painters.iter().any(|p| matches!(p, GaugePainter::VerticalBarGauge)));
    assert!(painters.iter().any(|p| matches!(p, GaugePainter::BasicReadout)));
    assert!(painters.iter().any(|p| matches!(p, GaugePainter::AsymmetricSweepGauge)));
}
...
<dashboard>
  <gaugeCluster>
    ...
    assert!(result.is_ok(), "Should parse minimal gauge: {:?}", result.err());
...
<gauge>
  <dashComp type="Gauge">
    ...
    assert!(result.is_ok(), "Should parse .gauge file: {:?}", result.err());

...
impl TuneFile {
    ...
    fn parse_msq(content: &str) -> io::Result<Self> {
        ...
        fn parse_value(value_str: &str) -> TuneValue {
            ...
            let was_quoted = trimmed.starts_with('"') && trimmed.ends_with('"') && trimmed.len() >= 2;
            ...
            if was_quoted {
                ...
                if let Ok(val) = unquoted.parse::<f64>() {
                    ...
                    if unquoted.chars().all(|c| c.is_ascii_digit() || c == '.' || c == '-' || c == '+') {
            ...
            if unquoted.starts_with('[') && unquoted.ends_with(']') {
                ...
                let parts: Vec<&str> = unquoted.split(|c: char| c.is_whitespace() || c == ',')
                    .filter(|s| !s.is_empty())
                    .collect();
                let values: Vec<f64> = parts.iter()
                    .filter_map(|s| s.parse::<f64>().ok())
                    .collect();
                    ...
                    while let Some(const_start) = page_content[const_pos..].find("<constant") {
                        ...
                        if let Some(name_attr_start) = const_remaining.find("name=\"") {
                            ...
                            if let Some(name_end) = const_remaining[name_start..].find('"') {
                                let name = const_remaining[name_start..name_start + name_end].to_string();
                                
                                if let Some(tag_end) = const_remaining.find('>') {
                                    let value_start = tag_end + 1;
                                    if let Some(close_tag) = const_remaining[value_start..].find("</constant>") {
                                        let value_str = const_remaining[value_start..value_start + close_tag].trim();
                    ...
                    while let Some(pc_start) = page_content[pc_pos..].find("<pcVariable") {
                        ...
                        if let Some(name_attr_start) = pc_remaining.find("name=\"") {
                            ...
                            if let Some(name_end) = pc_remaining[name_start..].find('"') {
                                let name = pc_remaining[name_start..name_start + name_end].to_string();
                                
                                if let Some(tag_end) = pc_remaining.find('>') {
                                    let value_start = tag_end + 1;
                                    if let Some(close_tag) = pc_remaining[value_start..].find("</pcVariable>") {
                                        let value_str = pc_remaining[value_start..value_start + close_tag].trim();
                                        let value = parse_value(value_str);
                                        tune.constants.insert(name.clone(), value);
                                        tune.constant_pages.insert(name, current_page);
                                        pcvars_found += 1;
        ...
        eprintln!("[DEBUG] parse_msq: Extracted {} constants and {} pcVariables from MSQ file", constants_found, pcvars_found);
    ...
    fn save_msq<P: AsRef<Path>>(&self, path: P) -> io::Result<()> {
        ...
        xml.push_str(&format!("  <versionInfo signature=\"{}\"/>\n", self.signature));
        ...
        for name in self.constants.keys() {
            ...
            constants_by_page.entry(page).or_insert_with(Vec::new).push(name.clone());
        }

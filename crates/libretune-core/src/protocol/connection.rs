...
use crate::ini::{ProtocolSettings, Endianness, AdaptiveTiming, AdaptiveTimingConfig};
use super::{
    ProtocolError, Command, Packet, PacketBuilder, CommandBuilder,
    serial::{open_port, configure_port, clear_buffers, PortInfo, list_ports},
    commands::{ReadMemoryParams, WriteMemoryParams, BurnParams},
...
fn write_and_wait(port: &mut Box<dyn SerialPort>, data: &[u8], baud_rate: u32, min_wait_ms: Option<u64>) -> Result<(), std::io::Error> {
    ...
    eprintln!("[DEBUG] write_and_wait: wrote {} bytes, waiting {}ms for transmission (baud={}, min={})", 
             data.len(), wait_ms, safe_baud, min_ms);
...
fn write_and_wait(port: &mut Box<dyn SerialPort>, data: &[u8], _baud_rate: u32, _min_wait_ms: Option<u64>) -> Result<(), std::io::Error> {
    port.write_all(data)?;
    port.flush()
}
...
impl Connection {
    ...
    pub fn with_protocol(config: ConnectionConfig, protocol: ProtocolSettings, endianness: Endianness) -> Self {
    ...
    pub fn enable_adaptive_timing(&mut self, config: Option<AdaptiveTimingConfig>) {
        ...
        eprintln!("[INFO] Adaptive timing enabled (multiplier={:.1}x, range={}–{}ms)", 
    ...
    pub fn adaptive_timing_stats(&self) -> Option<(Duration, usize)> {
        self.adaptive_timing.as_ref().and_then(|t| {
            t.average_response_time().map(|avg| (avg, t.sample_count()))
        })
    }
    
    /// Check if adaptive timing is enabled
    pub fn is_adaptive_timing_enabled(&self) -> bool {
        self.adaptive_timing.as_ref().map(|t| t.is_enabled()).unwrap_or(false)
    }
    ...
    fn get_effective_timeout(&self) -> Duration {
        ...
        let timeout_ms = self.protocol_settings
            .as_ref()
    ...
    fn get_effective_inter_char_timeout(&self) -> Duration {
        ...
        let base_ms = self.protocol_settings
            .as_ref()
            .map(|p| p.block_read_timeout as u64)
            .unwrap_or(1000);
    ...
    pub fn connect(&mut self) -> Result<(), ProtocolError> {
        ...
        let port_open_delay = self.protocol_settings
            .as_ref()
            .map(|p| p.delay_after_port_open)
            .unwrap_or(1000);
        eprintln!("[DEBUG] connect: waiting {}ms after port open for ECU stabilization (from INI)", port_open_delay);
    ...
    fn handshake(&mut self) -> Result<String, ProtocolError> {
        ...
        let query_cmd = self.protocol_settings
            .as_ref()
            .map(|p| p.query_command.clone())
            .unwrap_or_else(|| "S".to_string());
        
        // Check if INI specifies modern CRC protocol
        let ini_uses_modern = self.protocol_settings
            .as_ref()
            .map(|p| p.uses_modern_protocol())
            .unwrap_or(false);
        
        eprintln!("[DEBUG] handshake: query_cmd = {:?}, ini_uses_modern = {}", query_cmd, ini_uses_modern);
        ...
        if ini_uses_modern {
            ...
            match self.send_packet(packet) {
                Ok(response_packet) => {
                    ...
                    eprintln!("[DEBUG] handshake: CRC success, signature = {:?}", signature);
                    return Ok(signature);
                }
                Err(e) => {
                    eprintln!("[DEBUG] handshake: CRC protocol failed ({:?}), trying legacy", e);
                }
            }
        }
        
        // Try legacy protocol (raw ASCII command)
        eprintln!("[DEBUG] handshake: trying legacy protocol, sending byte 0x{:02x}", cmd_byte);
        ...
        match self.send_raw_command(&[cmd_byte]) {
            Ok(response) => {
                eprintln!("[DEBUG] handshake: legacy succeeded, {} bytes", response.len());
                self.use_modern_protocol = false;
                let signature = String::from_utf8_lossy(&response).trim().to_string();
                eprintln!("[DEBUG] handshake: legacy success, signature = {:?}", signature);
                return Ok(signature);
            }
            Err(e) => {
                ...
                if !ini_uses_modern {
                    ...
                    if let Ok(response_packet) = self.send_packet(packet) {
                        ...
                        let signature = String::from_utf8_lossy(signature_bytes).trim().to_string();
                        eprintln!("[DEBUG] handshake: CRC fallback success, signature = {:?}", signature);
                        return Ok(signature);
                    }
                }
    ...
    fn send_raw_command(&mut self, cmd: &[u8]) -> Result<Vec<u8>, ProtocolError> {
        ...
        let poll_interval = if self.is_adaptive_timing_enabled() { 1 } else { 2 };
        ...
        eprintln!("[DEBUG] send_raw_command: sending {} bytes: {:02x?}", cmd.len(), cmd);
        ...
        eprintln!("[DEBUG] send_raw_command: command sent, timeout={}ms, inter_char={}ms", 
                 timeout.as_millis(), inter_char_timeout.as_millis());
            ...
            if available > 0 {
                ...
                match port.read(&mut buffer[..to_read]) {
                    ...
                    Ok(n) => {
                        ...
                        eprintln!("[DEBUG] send_raw_command: read {} bytes, total = {}, data = {:02x?}", 
                                 n, response.len(), &buffer[..n]);
                    }
                ...
                if last_data_time.elapsed() > inter_char_timeout {
                    eprintln!("[DEBUG] send_raw_command: inter-character timeout, message complete");
                    break;
                }
                std::thread::sleep(Duration::from_millis(poll_interval));
            }
        ...
        eprintln!("[DEBUG] send_raw_command: completed with {} bytes in {}ms: {:?}", 
                 response.len(), elapsed.as_millis(), String::from_utf8_lossy(&response));
    ...
    fn send_raw_command_no_response(&mut self, cmd: &[u8]) -> Result<(), ProtocolError> {
        ...
        eprintln!("[DEBUG] send_raw_command_no_response: sending {} bytes: {:02x?}", cmd.len(), cmd);
    ...
    fn send_packet_no_response(&mut self, packet: Packet) -> Result<(), ProtocolError> {
        ...
        eprintln!("[DEBUG] send_packet_no_response: sending {} bytes", bytes.len());
    ...
    fn send_packet(&mut self, packet: Packet) -> Result<Packet, ProtocolError> {
        ...
        let min_wait = Some(self.get_effective_min_wait());
        let timeout = self.get_effective_timeout();
        let poll_interval_ms = if self.is_adaptive_timing_enabled() { 1 } else { 2 };
        ...
        let bytes = packet.to_bytes();
        eprintln!("[DEBUG] send_packet: sending {} bytes: {:02x?}", bytes.len(), bytes);
        // Use write_and_wait which avoids the blocking tcdrain issue
        write_and_wait(port, &bytes, baud_rate, min_wait)
            .map_err(|e| ProtocolError::SerialError(e.to_string()))?;
        
        eprintln!("[DEBUG] send_packet: packet sent, timeout={}ms", timeout.as_millis());
        
        // Helper to read exact bytes with timeout
        // Uses bytes_to_read() polling to avoid blocking read() calls on Linux
        fn read_exact_timeout(port: &mut Box<dyn SerialPort>, buf: &mut [u8], timeout: Duration, poll_ms: u64) -> Result<(), ProtocolError> {
            let start = Instant::now();
            let mut offset = 0;
            
            while offset < buf.len() {
                if start.elapsed() > timeout {
                    eprintln!("[DEBUG] read_exact_timeout: timed out after reading {} of {} bytes", offset, buf.len());
                    return Err(ProtocolError::Timeout);
                }
                ...
                match port.read(&mut buf[offset..offset + to_read]) {
                    ...
                    Ok(n) => {
                        eprintln!("[DEBUG] read_exact_timeout: read {} bytes: {:02x?}", n, &buf[offset..offset+n]);
                        offset += n;
                    }
                    Err(ref e) if e.kind() == std::io::ErrorKind::TimedOut 
                               || e.kind() == std::io::ErrorKind::WouldBlock => {
        ...
        if let Err(e) = read_exact_timeout(port_box, &mut payload_and_crc, timeout, poll_interval_ms) {
            self.reset_adaptive_timing_on_error();
            return Err(e);
        }
    ...
    pub fn get_realtime_data(&mut self) -> Result<Vec<u8>, ProtocolError> {
        ...
        let burst_cmd = self.protocol_settings
            .as_ref()
    ...
    pub fn read_memory(&mut self, params: ReadMemoryParams) -> Result<Vec<u8>, ProtocolError> {
        ...
        let read_format = self.protocol_settings
            .as_ref()
            .and_then(|p| p.page_read_commands.get(page).cloned())
            .unwrap_or_else(|| "R%2i%2o%2c".to_string());
        
        if read_format.is_empty() {
            return Err(ProtocolError::ProtocolError(format!("No read command for page {}", page)));
        }
        ...
        if self.use_modern_protocol {
            ...
            if status != 0 {
                return Err(ProtocolError::ProtocolError(format!("Read error, status: {}", status)));
            }
    ...
    pub fn read_page(&mut self, page: u8) -> Result<Vec<u8>, ProtocolError> {
        let page_size = self.protocol_settings
            .as_ref()
            .and_then(|p| p.page_sizes.get(page as usize).copied())
            .unwrap_or(0);
        
        if page_size == 0 {
            return Err(ProtocolError::ProtocolError(format!("Unknown page size for page {}", page)));
        }
        
        let blocking_factor = self.protocol_settings
            .as_ref()
    ...
    pub fn write_memory(&mut self, params: WriteMemoryParams) -> Result<(), ProtocolError> {
        ...
        let write_format = self.protocol_settings
            .as_ref()
            .and_then(|p| p.page_chunk_write_commands.get(page).cloned())
            .unwrap_or_else(|| "C%2i%2o%2c%v".to_string());
        
        if write_format.is_empty() {
            return Err(ProtocolError::ProtocolError(format!("No write command for page {}", page)));
        }
    ...
    pub fn burn(&mut self, params: BurnParams) -> Result<(), ProtocolError> {
        ...
        let burn_format = self.protocol_settings
            .as_ref()
        ...
        if burn_format.is_empty() {
            eprintln!("[DEBUG] burn: page {} has empty burn command, skipping", page);
            return Ok(());
        }
        
        // Build command using INI format string (use page_id, not page index)
        let cmd = self.command_builder.build_burn_command(
            &burn_format,
            page_id,
        )?;
        
        eprintln!("[DEBUG] burn: sending burn command for page {}, format='{}', cmd = {:02x?}", page, burn_format, cmd);
        ...
        let delay = self.protocol_settings
            .as_ref()
            .map(|p| p.page_activation_delay.max(2000))
            .unwrap_or(2000);
        
        eprintln!("[DEBUG] burn: waiting {}ms for flash write to complete", delay);

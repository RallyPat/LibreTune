    ...
    match EcuDefinition::from_file(full_path.to_string_lossy().as_ref()) {
        Ok(def) => {
            ...
            *evaluator_guard = Some(evaluator);
            ...
            // Build TuneCache locally WITHOUT holding the state lock.
            // This prevents blocking concurrent get_table_data calls that need tune_cache.
                    ...
                    if let Some(constant) = def_clone.constants.get(name) {
                        ...
                        if constant.data_type == libretune_core::ini::DataType::Bits {
                            ...
                            let bit_value = match tune_value {
                                ...
                                TuneValue::Bool(b) => if *b { 1 } else { 0 },
                                TuneValue::String(s) => {
                                    if let Some(index) =
                            ...
                            if bits_remaining_after_first_byte > 0 {
                                ...
                                for i in 1..bytes_needed_usize.min(current_bytes.len()) {
                                    ...
                                    let mask = if bits_from_this_byte >= 8 { 0xFF } else { (1u8 << bits_from_this_byte) - 1 };
                        ...
                        match tune_value {
                            TuneValue::Scalar(v) => {
                                ...
                                constant.data_type.write_to_bytes(
                                    &mut raw_data, 0, raw_val, def_clone.endianness,
                                );
                            ...
                            TuneValue::Array(arr) => {
                                ...
                                for i in 0..element_count {
                                    ...
                                    constant.data_type.write_to_bytes(
                                        &mut raw_data, i * element_size, raw_val, def_clone.endianness,
                                    );
                                }
                                if cache.write_bytes(constant.page, constant.offset, &raw_data) {
                                    applied_count += 1;

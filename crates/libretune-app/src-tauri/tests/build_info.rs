...
fn test_build_id_format() {
    ...
    if build_id != "unknown" {
        ...
        assert_eq!(parts.len(), 2, "Build ID should have date and git parts separated by +");
        ...
        assert_eq!(date_pieces.len(), 3, "Date should have 3 components (YYYY.MM.DD)");
        
        // Year should be 4 digits
        assert_eq!(date_pieces[0].len(), 4, "Year should be 4 digits");
        date_pieces[0].parse::<u16>().expect("Year should be valid number");
        
        // Month should be 2 digits
        assert_eq!(date_pieces[1].len(), 2, "Month should be 2 digits");
        let month: u8 = date_pieces[1].parse().expect("Month should be valid number");

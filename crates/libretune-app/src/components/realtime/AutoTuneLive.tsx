...
export default function AutoTuneLive({ onClose }: { onClose: () => void }) {
  ...
  const getCellColor = (value: number, recommended: number) => {
    if (value === recommended) return '#4CAF50';
    const diff = recommended - value;
    const alpha = Math.min(Math.abs(diff) / 5, 1);
    if (diff > 0) {
  ...
  const getHeatMapValue = (key: string, type: 'weighting' | 'change') => {
    const value = heatMapData[type].get(key) || 0;
    const max = Math.max(...Array.from(heatMapData[type].values()));
    const ratio = value / (max || 1);
    if (type === 'weighting') {
      const green = Math.floor(ratio * 240);
      return `hsl(${green}, 60%, 45%)`;
    } else {
      const hue = 240 - Math.floor(ratio * 120);
